...
**Audience:** Frontend developers building the employer portal (dashboard, jobs, applicants, workforce, payments, company)

Verified against production with `npm run smoke:employer` (May 23, 2026).

**Related:** Admin moderation uses `/admin/*` — see [FRONTEND_ADMIN_PANEL_API_GUIDE_MAY_2026.md](./FRONTEND_ADMIN_PANEL_API_GUIDE_MAY_2026.md). Auth is shared at `/auth/*`.
...
4. [Route index (30 routes)](#4-route-index-30-routes)
...
12. [Smoke tests & env vars](#12-smoke-tests--env-vars)
...
{
  "identifier": "employer@company.cm",
...
```json
{
  ...
  "user": {
    "id": "uuid",
    "email": "employer@company.cm",
    "role": "EMPLOYER"
  }
}
```

**Errors:** `401` invalid credentials; `403` if account is not an employer.

Registration flow (employer signup UI):

1. `POST /auth/register` — `{ email, password, role: "EMPLOYER", languagePreference: "EN" }`
2. `POST /auth/verify` — `{ identifier, otp, role, password, languagePreference }`
3. Store `accessToken` from verify response

On production, OTP is sent by email/SMS. **`JOBALLA_DEV_FIXED_OTP`** exists only for non-production dev servers.
...
2. `GET /api/employer/me` on app load
3. On **401**, try refresh once, then redirect to login
4. Attach `Authorization: Bearer <token>` to every `/api/employer/*` call
...
| 14 | GET | `/api/employer/applicants/:applicationId/share` | Shareable profile link |
| 15 | GET | `/api/employer/workforce` | Hired workers list + stats |
| 16 | GET | `/api/employer/workforce/:workerId` | Worker detail |
| 17 | GET | `/api/employer/workforce/:workerId/shifts` | Shift log |
| 18 | POST | `/api/employer/workforce/:workerId/shifts` | Log a shift |
| 19 | PATCH | `/api/employer/workforce/:workerId/shifts/:shiftId` | Edit shift |
| 20 | DELETE | `/api/employer/workforce/:workerId/shifts/:shiftId` | Delete shift |
| 21 | PATCH | `/api/employer/workforce/:workerId/status` | Terminate / reinstate |
| 22 | GET | `/api/employer/payments` | Payroll summary cards |
| 23 | GET | `/api/employer/payments/workers` | Workers pay table |
| 24 | POST | `/api/employer/payments/pay` | Pay one worker |
| 25 | GET | `/api/employer/payments/history` | Payment history |
| 26 | GET | `/api/employer/payments/:paymentId` | Payment detail |
| 27 | GET | `/api/employer/payments/statement` | Statement data (PDF hook) |
| 28 | GET | `/api/employer/company` | Company profile |
| 29 | PATCH | `/api/employer/company` | Update company |
| 30 | POST | `/api/employer/company/logo` | Upload logo (multipart) |
...
| | |
|---|---|
| **Query** | `status` (see [§11](#11-status-reference)), `page`, `limit` |
| **Auth** | Bearer |
...
### 7.5 `GET /api/employer/applicants/:applicationId/share`
...
Company name, industry, size, bio, location, website, logo URL, verification fields.
...
**Success (`200`):** Updated company object.
...
## 12. Smoke tests & env vars

Production default: **`https://joballa-api.onrender.com`**
