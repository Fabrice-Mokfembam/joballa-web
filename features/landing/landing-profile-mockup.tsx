...
function MockField({ label, value, placeholder }: { label: string; value?: string; placeholder?: string }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-1">
      ...
      <div className="flex h-9 items-center rounded-xl border border-[var(--landing-border-muted)] bg-[var(--landing-mockup-input-bg)] px-3 text-sm text-[var(--landing-mockup-fg)]">
        {value ?? <span className="text-[var(--landing-fg-muted)]">{placeholder}</span>}
      </div>
    </div>
  );
}
...
function MockSelect({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-1">
      <label className="px-1 text-xs font-medium text-[var(--landing-fg-muted)]">{label}</label>
      <div className="flex h-9 items-center justify-between rounded-xl border border-[var(--landing-border-muted)] bg-[var(--landing-mockup-input-bg)] px-2.5 text-sm text-[var(--landing-mockup-fg)]">
        <span>{value}</span>
...
export function LandingProfileMockup() {
  return (
    <div className="rounded-[14px] border border-[var(--landing-border-muted)] bg-[var(--landing-mockup-bg)] p-6">
      <p className="text-xs font-bold uppercase tracking-normal text-[var(--landing-fg-muted)]">Personal info</p>
      <div className="mt-1.5 flex gap-2">
        <div className="flex shrink-0 flex-col items-center gap-1.5 px-2 py-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-7 py-4 pl-4">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex h-9 items-center justify-between rounded-xl border border-[var(--landing-border-muted)] bg-[var(--landing-mockup-input-bg)] px-2.5 text-sm text-[var(--landing-fg-muted)]">
                <span>Ex: English, French, Spanish</span>
                <ChevronDown className="size-4 shrink-0" aria-hidden />
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-6 w-11 rounded-full bg-[var(--landing-border-muted)] p-0.5">
