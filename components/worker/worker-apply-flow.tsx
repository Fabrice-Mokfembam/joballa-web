...
export function WorkerApplyFlow({
  ...
  const [customization, setCustomization] = useState<ApplicationProfileCustomization | null>(null);
  ...
  useEffect(() => {
    if (draftHydrated || !profile) return;
    const fromLocal = readApplicationProfileDraft(jobId);
    setCustomization(fromLocal ?? buildApplicationCustomizationFromProfile(profile));
    setDraftHydrated(true);
  }, [draftHydrated, jobId, profile]);

  function openCustomize() {
    if (profile && !customization) {
      setCustomization(buildApplicationCustomizationFromProfile(profile));
    }
